...
//   energy.arrow    the schema and one record batch per temperature drawn
...
//
// Every material belongs to one library and one temperature, so the same
// material at 294 K and at 900 K is two entries here, each holding the
// batches for its own temperature. A temperature is only ever one the data
// publishes: the wasm can blend between two of them, but it does so silently
// from one side when only one neighbour has been downloaded, so the page
// offers the published ladder and nothing between.
//
...
import {
  reactionRanges, energyRanges, publishedMts, planFetch, coalesce, rangeHeader, EOS,
} from './ranges.js';
import { LIBRARIES, DEFAULT_LIBRARY } from './libraries.js';
import { DEFAULT_TEMPERATURE, isPublished, bareLabel } from './temperatures.js';
...
export function createEngine({ origin = ORIGIN, fetchImpl = fetch } = {}) {
  ...
  /// Load a nuclide's metadata from one library and record where its grids
  /// and reactions live, so each can be fetched when a temperature and a
  /// reaction are chosen.
  function ensureNuclide(library, name) {
    const k = nuclideKey(library, name);
    if (!nuclides.has(k)) {
  ...
  async function loadNuclide(library, name) {
    ...
    const entry = {
      ...
      fileVersion: 0,
      /// The MTs the whole-file path found, across every temperature. While
      /// the byte-range index is in use the answer depends on the temperature
      /// and comes from `publishedAt` instead.
      mts: null,
      whole: false,
      wholeEnergy: false,
      schema: null,
      ranges: null,
      /// The byte-range index of energy.arrow, and the grids fetched from it.
      energy: null,
      energySchema: null,
      grids: new Map(),
    };
    ...
    if (reactions && energy && !rangesStripped) {
      // Both indexes are usable, so nothing else is fetched until a
      // temperature and a reaction are asked for.
      entry.ranges = reactions;
      entry.energy = energy;
    } else {
      // No usable index, or an origin that ignores Range: take both files
      // whole. They carry every temperature, so nothing more is needed.
      const got = await fetchBytes(url(library, name, 'energy.arrow'));
      sim.add_file(`${entry.dir}/energy.arrow`, got.bytes);
      entry.wholeEnergy = true;
  ...
  /// The MTs this nuclide publishes at one temperature.
  function publishedAt(entry, temperature) {
    if (entry.whole) return entry.mts;
    return new Set(publishedMts(entry.ranges, temperature));
  }

  /// Make sure the energy grid for `temperature` is in the wasm filesystem.
  ///
  /// Every cross section is interpolated against this grid, so it is fetched
  /// before the reactions that need it, once per temperature per nuclide.
  function ensureGrid(entry, temperature) {
    if (entry.wholeEnergy || entry.grids.has(temperature)) return Promise.resolve();
    const running = entry.inFlight.get(`grid@${temperature}`);
    if (running) return running;
    const job = fetchGrid(entry, temperature)
      .finally(() => entry.inFlight.delete(`grid@${temperature}`));
    entry.inFlight.set(`grid@${temperature}`, job);
    return job;
  }

  async function fetchGrid(entry, temperature) {
    const range = entry.energy?.temperatures.get(temperature);
    if (!range) throw new Error(`${entry.name} is not published at ${temperature} in ${entry.library}`);
    const target = url(entry.library, entry.name, 'energy.arrow');
    // The schema is fetched once and kept, as it is for the reactions file.
    const parts = entry.energySchema ? [range] : [entry.energy.schema, range];
    ...
    if (whole) {
      // The origin sent the whole file, which carries every temperature.
      sim.add_file(`${entry.dir}/energy.arrow`, whole.bytes);
      entry.wholeEnergy = true;
      entry.grids.clear();
      entry.fileVersion++;
      return;
    ...
    if (!entry.energySchema) entry.energySchema = cut(entry.energy.schema);
    entry.grids.set(temperature, { off: range.off, len: range.len, bytes: cut(range) });
    writeEnergy(entry);
  }

  /// Rewrite energy.arrow from the grids fetched so far: schema, grids in file
  /// order, end of stream. The shape reactions.arrow is rewritten in too.
  function writeEnergy(entry) {
    const parts = [...entry.grids.values()].sort((a, b) => a.off - b.off);
    const total = entry.energySchema.length + parts.reduce((n, p) => n + p.len, 0) + EOS.length;
    const out = new Uint8Array(total);
    out.set(entry.energySchema, 0);
    let at = entry.energySchema.length;
    for (const p of parts) {
      out.set(p.bytes, at);
      at += p.len;
    }
    out.set(EOS, at);
    sim.add_file(`${entry.dir}/energy.arrow`, out);
    entry.fileVersion++;
  }

  /// Make sure the reactions file for `entry` carries every MT in `wanted`
  /// that the nuclide publishes at `temperature`.
  async function ensureReactions(entry, wanted, temperature) {
    if (entry.whole) return;
    const available = publishedAt(entry, temperature);
    const missing = [];
    const pending = [];
    for (const mt of wanted) {
      if (!available.has(mt)) continue;
      const at = `${mt}@${temperature}`;
      if (entry.batches.has(at)) continue;
      if (entry.inFlight.has(at)) pending.push(entry.inFlight.get(at));
      else missing.push(mt);
    }
    if (missing.length) {
      const job = fetchReactions(entry, missing, temperature);
      for (const mt of missing) entry.inFlight.set(`${mt}@${temperature}`, job);
      pending.push(job);
    }
    await Promise.all(pending);
  }

  async function fetchReactions(entry, mts, temperature) {
    const { ranges } = entry;
    ...
    try {
      ...
      for (const part of wanted) {
        // One batch is one MT at one temperature, so both belong in the key:
        // the same reaction at 294 K and at 900 K is two batches to hold.
        entry.batches.set(`${part.mt}@${temperature}`, { off: part.off, len: part.len, bytes: cut(part) });
      }
      writeReactions(entry);
    } finally {
      for (const mt of mts) entry.inFlight.delete(`${mt}@${temperature}`);
    }
  }
  ...
  /// Register a material from one library at one temperature, and return the
  /// MTs it can be plotted for: the union of what its nuclides publish at that
  /// temperature, read from the indexes without touching the wasm.
  async function createMaterial(materialId, def, library = DEFAULT_LIBRARY, temperature = DEFAULT_TEMPERATURE) {
    checkLibrary(library);
    checkTemperature(temperature);
    const k = key(library, materialId, temperature);
    ...
    // Every cross section is interpolated against the grid for this
    // temperature, so it is in hand before any reaction is.
    await Promise.all(entries.map((entry) => ensureGrid(entry, temperature)));
    const mts = new Set();
    for (const entry of entries) for (const mt of publishedAt(entry, temperature)) mts.add(mt);
    const material = {
      def, library, temperature, entries,
    ...
    for (const entry of entries) ensureReactions(entry, [TOTAL_MT], temperature).catch(() => {});
    return material.mts;
  }
  ...
  function buildMaterial(material) {
    ...
    try {
      ...
      wasm.set_temperature(bareLabel(material.temperature));
    } catch (err) {
      wasm.free();
      throw err;
    }
  ...
  /// Macroscopic cross sections of `materialId` from `library` at
  /// `temperature`, for each MT in `mts`, on the material's unified grid.
  async function calculateXs(materialId, mts, library = DEFAULT_LIBRARY, temperature = DEFAULT_TEMPERATURE) {
    checkLibrary(library);
    checkTemperature(temperature);
    const material = materials.get(key(library, materialId, temperature));
    if (!material) throw new Error(`material ${materialId} from ${library} at ${temperature} not created`);
    mts = [...new Set(mts.map(Number))];
    const wanted = new Set([TOTAL_MT, ...mts]);
    await Promise.all(material.entries.map((entry) => ensureReactions(entry, wanted, temperature)));
    ...
    let energyGrid = null;
    for (const mt of mts) {
      // Guarded, not tried: an MT no nuclide of this material carries at this
      // temperature would panic inside the wasm and take the engine with it.
      if (!material.entries.some((e) => publishedAt(e, temperature).has(mt))) {
        throw new Error(`MT ${mt} is not available for material ${materialId} in ${library} at ${temperature}`);
      }
      const [xs, energy] = material.wasm.macroscopicCrossSection(mt, bareLabel(temperature));
      crossSections[mt] = xs;
